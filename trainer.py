...
class Trainer(object):
    ...
    def load_symbol2id(self):
        # 将实体和关系转换为索引id，无需embedding
        symbol_id = {}
        rel2id = json.load(open(self.dataset + '/relation2ids'))
        ent2id = json.load(open(self.dataset + '/ent2ids'))
        i = 0
        # 将实体和关系的表示合并在一起，形成一个自增的索引数组
    ...
    def train(self):
        ...
        # return [support_pairs, query_pairs, false_pairs, support_left, support_right, query_left, query_right, false_left, false_right]
        for data in train_generate(self.dataset, self.batch_size, self.train_few, self.symbol2id, self.ent2id,self.id2ent,self.rel2id,
                                   self.e1rel_e2,self.dom_ent,self.ent_dom,self.rel2dom_h,self.rel2nn):
            # [2], [batchsize,2], [batchsize,2]...
